...
const SignUp = () => {
    ...
    const [profilepic, setProfilePic] = useState<string>("https://res.cloudinary.com/dhz4wb76m/image/upload/c_pad,b_auto:predominant,fl_preserve_transparency/v1688671165/profile_images/qdq7bnpm9lqqbo3ufmc9.jpg?_s=public-apps")
    const [role, setRole] = useState<string>("")
    const [phoneNumber, setPhoneNumber] = useState<string>("")
    const [coverpic, setCoverPic] = useState<string>("https://notepd.s3.amazonaws.com/default-cover.png")
    const router = useRouter()
    //!signup function for client
    const handleSingUpClient = () => {
        axios.post("http://localhost:5000/client/signup", { firstname, lastname, username, email, password, profilepic, role, phoneNumber, coverpic })
            .then((res: AxiosResponse) => console.log(res))
            .catch((err: AxiosError) => console.log(err))
    }
    //!signup function for seller
    const handleSingUpSeller = () => {
        axios.post("http://localhost:5000/seller/signup", { firstname, lastname, username, email, password, profilepic, role, phoneNumber, coverpic })
            .then((res: AxiosResponse) => console.log(res))
            .catch((err: AxiosError) => console.log(err))
    }
